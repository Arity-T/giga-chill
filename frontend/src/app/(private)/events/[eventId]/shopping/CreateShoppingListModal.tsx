...
import { useCreateShoppingListMutation } from '@/store/api/api';
...
interface CreateShoppingListFormData {
    title: string;
    description: string;
}

export default function CreateShoppingListModal({ open, onCancel, eventId }: CreateShoppingListModalProps) {
    const [form] = Form.useForm<CreateShoppingListFormData>();
    const [createShoppingList, { isLoading }] = useCreateShoppingListMutation();
    const { message } = App.useApp();

    const handleSubmit = async (values: CreateShoppingListFormData) => {
        try {
