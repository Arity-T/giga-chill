...
public class TaskServiceLoggerAspect {
    ...
    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.getAllTasksFromEvent(..)) " +
            "&& args(eventId)")
    ...
    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.updateTask(..)) " +
            "&& args(taskId, ..)")
    public void updateTask(UUID taskId) {
    }

    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.startExecuting(..)) " +
            "&& args(taskId, userId)")
    ...
    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.getTaskStatus(..)) " +
            "&& args(taskId)")
    public void getTaskStatus(UUID taskId) {
    }

    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.isExisted(..)) " +
            "&& args(eventID, taskId)")
    public void isExisted(UUID eventID, UUID taskId) {
    }

    @Pointcut("execution(public * com.github.giga_chill.gigachill.service.TaskService.canExecute(..)) " +
            "&& args(taskId, userId)")
    public void canExecute(UUID taskId, UUID userId) {
    }

    @Around("getAllTasksFromEvent(eventId)")
    public Object logGetAllTasksFromEvent(ProceedingJoinPoint proceedingJoinPoint,
                                          UUID eventId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getGET_COLOR() + "Event tasks with id: {} received"
    ...
    public Object logCreateTask(ProceedingJoinPoint proceedingJoinPoint,
                                UUID eventId,
                                User user) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getPOST_COLOR() + "User with id: {} created task with id: " +
                    "{} in event with id: {}"
                    + loggerColorConfig.getRESET_COLOR(), user.getId(), (String) result, eventId);
            return result;
        } catch (Throwable ex) {
            throw ex;
        }
    ...
    public Object logUpdateTask(ProceedingJoinPoint proceedingJoinPoint,
                                UUID taskId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getPATCH_COLOR() + "Task with id: {} was updated"
    ...
    public Object logStartExecuting(ProceedingJoinPoint proceedingJoinPoint,
                                    UUID taskId,
                                    UUID userId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getPOST_COLOR() + "User with id: {} started execution task with id: {}"
                    + loggerColorConfig.getRESET_COLOR(), userId, taskId);
            return result;
        } catch (Throwable ex) {
            throw ex;
        }
    ...
    public Object logDeleteTask(ProceedingJoinPoint proceedingJoinPoint,
        ...
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getDELETE_COLOR() + "Task with id: {} was deleted"
                    + loggerColorConfig.getRESET_COLOR(), taskId);
            return result;
        } catch (Throwable ex) {
            throw ex;
    ...
    public Object logIsAuthor(ProceedingJoinPoint proceedingJoinPoint,
                                UUID taskId,
                                UUID userId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            if ((Boolean) result) {
                LOGGER.info(loggerColorConfig.getGET_COLOR() + "User with id: {} is author of task with id: {}"
                                + loggerColorConfig.getRESET_COLOR(),
                        userId, taskId);
            } else {
                LOGGER.info(loggerColorConfig.getGET_COLOR() + "User with id: {} is not author of task with id: {}"
                                + loggerColorConfig.getRESET_COLOR(),
                        userId, taskId);
            }
            return result;
    ...
    public Object logGetTaskStatus(ProceedingJoinPoint proceedingJoinPoint,
                                UUID taskId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            LOGGER.info(loggerColorConfig.getDELETE_COLOR() + "Task with id: {} has status {}"
                    + loggerColorConfig.getRESET_COLOR(), taskId, (String) result);
            return result;
        } catch (Throwable ex) {
            throw ex;
        }
    ...
    public Object logCanExecute(ProceedingJoinPoint proceedingJoinPoint,
                                UUID taskId,
                                UUID userId) throws Throwable {
        try {
            Object result = proceedingJoinPoint.proceed();
            if ((Boolean) result) {
                LOGGER.info(loggerColorConfig.getGET_COLOR() + "User with id: {} can execute task with id: {}"
                                + loggerColorConfig.getRESET_COLOR(),
                        userId, taskId);
            } else {
                LOGGER.info(loggerColorConfig.getGET_COLOR() + "User with id: {} can not execute task with id: {}"
                                + loggerColorConfig.getRESET_COLOR(),
                        userId, taskId);
            }
            return result;
